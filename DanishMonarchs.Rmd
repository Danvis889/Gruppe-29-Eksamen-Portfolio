...
read_csv("data/Kongerækken.csv")
```

The task here is to load your Danish Monarchs csv into R the `tidyverse` toolkit, calculate and explore the kings' duration of reign with pipes `%>%` in `dplyr` and plot it over time.
...
# Removing interregnum and Frederick 10.

kings <- kings[-c(29,39,57), ]
...
kings %>% 
  summarise(mean_duration = mean(duration), min_duration = min(duration),max_duration = max(duration))
...
# YOUR CODE
kings %>% 
  filter(duration > 20.14815) %>% 
  group_by(king_index, navn, duration) %>% 
  arrange(duration) %>% 
  count(duration)
```
...
# Creating a new column for days
top3 %>% 
...
# Creating the midyear column
kings <- kings %>% 
  mutate(midyear = reg_start + (reg_slut - reg_start)/2)
...
kings %>% 
  ...
  geom_point() +
  geom_smooth() +
  ggtitle("Duration of the Reigns of Danish Monarchs") +
  theme_bw() +
  theme(plot.title = element_text(size = 20, face = "bold"), axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16), axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12))
