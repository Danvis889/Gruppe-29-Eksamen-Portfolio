...
author: 'Daniel Sørensen
date: 'edited on 19 March 2025 and updated `r format(Sys.time(), "%d %B, %Y")`'---
...
"Due to the popular that A Game of Thrones contains violence, sex, and death, the sentiment analysis will portray words that contain strong negative connotations with strong emotions of fear, anger, etc."
...
### A word cloud of AGOT words (non-numeric)
...
Let's do sentiment analysis on the AGOT text data using afinn, and nrc. 
...
# OK so what's the deal with honor? And is it really "positive" in the emotion sense? 
```

Look back at AGOT, and search for "honor". Is it typically associated with emotion, or something else? 
...
Wait, so "honor" is showing up in NRC lexicon as "fear"? Let's check:
```{r nrc-honor}
