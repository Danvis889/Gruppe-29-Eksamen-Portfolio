...
```{r}
#I start out by making a new coloum called uratuoion_reign using mutate
kings <- kings %>% 
  filter(!is.na(første_regeringsår))%>% 
  filter(!is.na(sidste_regeringsår))%>% 
  mutate(duratuoion_reign = sidste_regeringsår - første_regeringsår) 
...
```{r}
# i then find the average using base R
mean(kings$duratuoion_reign, na.rm=TRUE)
...
```{r}
#Then i find the above average by first using the filter fuction and after that the count function to count how meny were above
above_avb_kings <- kings %>% 
  filter(duratuoion_reign >=19.16667)
count(above_avb_kings)
```
...
da vi i vores kildesæt har delt Chrisian 4 ind i formynderår og anden regeringsperiode, ses han ikke som den længst siddende monak i, som han ellers burde

```{r}
#then i find the three longest ruling monarchs by using the slice_max fuction 
#!!da vi i vores kildesæt har delt Chrisian 4 ind i formynderår og anden regeringsperiode, ses han ikke som den længst siddende monak i, som han ellers burde!!
kings %>% 
  slice_max(duratuoion_reign, n=3)
```
